...
class PrivateLinkAttachmentConnection(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if aws is not None and not isinstance(aws, PrivateLinkAttachmentConnectionAwsArgs):
                aws = aws or {}
                def _setter(key, value):
                    aws[key] = value
                PrivateLinkAttachmentConnectionAwsArgs._configure(_setter, **aws)
            __props__.__dict__["aws"] = aws
            if azure is not None and not isinstance(azure, PrivateLinkAttachmentConnectionAzureArgs):
                azure = azure or {}
                def _setter(key, value):
                    azure[key] = value
                PrivateLinkAttachmentConnectionAzureArgs._configure(_setter, **azure)
            __props__.__dict__["azure"] = azure
            __props__.__dict__["display_name"] = display_name
            if environment is not None and not isinstance(environment, PrivateLinkAttachmentConnectionEnvironmentArgs):
                environment = environment or {}
                def _setter(key, value):
                    environment[key] = value
                PrivateLinkAttachmentConnectionEnvironmentArgs._configure(_setter, **environment)
            if environment is None and not opts.urn:
                raise TypeError("Missing required property 'environment'")
            __props__.__dict__["environment"] = environment
            if gcp is not None and not isinstance(gcp, PrivateLinkAttachmentConnectionGcpArgs):
                gcp = gcp or {}
                def _setter(key, value):
                    gcp[key] = value
                PrivateLinkAttachmentConnectionGcpArgs._configure(_setter, **gcp)
            __props__.__dict__["gcp"] = gcp
            if private_link_attachment is not None and not isinstance(private_link_attachment, PrivateLinkAttachmentConnectionPrivateLinkAttachmentArgs):
                private_link_attachment = private_link_attachment or {}
                def _setter(key, value):
                    private_link_attachment[key] = value
                PrivateLinkAttachmentConnectionPrivateLinkAttachmentArgs._configure(_setter, **private_link_attachment)
